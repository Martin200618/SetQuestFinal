...
function crearSala() {
  console.log('click');
  socket.emit('crear-sala', nombreJugador);
}


function unirseASala(salaId) {
  socket.emit('unirse-a-sala', salaId, nombreJugador);
  salaId = salaId;
}


socket.on('lista-salas', (salas) => {
  console.log("Lista de salas:", salas);
  const listaSalasDiv = document.querySelector('.lista-salas');
  listaSalasDiv.innerHTML = ""; 

  for (let salaId in salas) {
    const sala = salas[salaId];
    const salaElement = document.createElement('div');
    salaElement.innerHTML = `<p>Sala ${salaId} - Jugadores: ${sala.jugadores.length}</p>`;
    salaElement.style = `display:flex;color:#fff;`
    ...
    unirseBtn.style = `
          ...
          z-index: 99;
    `;
    unirseBtn.onclick = () => unirseASala(salaId);

    salaElement.appendChild(unirseBtn);
    listaSalasDiv.appendChild(salaElement);
  }
});

socket.on('sala-creada', (salaId) => {
  console.log(salaId)
});


socket.on('nuevo-jugador', (jugador) => {
  naves[jugador.id] = new Nave(jugador.id, jugador.nombre);
});


socket.on('movimiento-jugador', (jugadorId, posicion) => {
  if (naves[jugadorId]) {
    naves[jugadorId].actualizarPosicion(posicion);
  }
});


socket.on('puntajes-actualizados', (jugadores) => {
  jugadores.forEach(jugador => {
    if (naves[jugador.id]) {
      naves[jugador.id].actualizarPuntaje(jugador.puntaje);
    }
  });
});
